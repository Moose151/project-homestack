...
from apps.notifications.tasks import run_countdown_digest, run_due_reminders, run_due_todo_offsets
...
class Command(BaseCommand):
    help = "Send due 24h/morning-of/To-do-offset reminders and the daily countdown digest."

    def handle(self, *args, **options):
        reminders = run_due_reminders()
        todo_offsets_sent = run_due_todo_offsets()
        digest = run_countdown_digest()
        self.stdout.write(
            self.style.SUCCESS(
                f"Reminders — 24h: {reminders['24h']}; morning_of: {reminders['morning_of']}. "
                f"To-do offsets sent: {todo_offsets_sent}. "
