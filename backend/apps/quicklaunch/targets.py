...
register(Target(
    key="core.calendar",
    label="Calendar",
    description="Events, appointments and reminders",
    icon="📅",
    node_key="",
    target_type="open",
...
register(Target(
    ...
    node_key="atlas",
    target_type="open",
    route=lambda user, obj=None: "/atlas?tab=reminders",
))


def _visible_lists(user):
    from apps.atlas import selectors
    return [(row.id, row.title) for row in selectors.list_atlas_lists(user)]
...
def _list_tab(list_type: str) -> str:
    """Mirrors the page's own listTabFor(): grocery and shopping have dedicated tabs."""
    return list_type if list_type in ("grocery", "shopping") else "lists"
