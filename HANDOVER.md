...
- [x] All architectural decisions made (D1–D18; **D19/D20 proposed — see M2**).
...
- [~] **Milestone 2: native Meridian — IN PROGRESS. ← active task.** Only a thin
  tasks/points/rewards foundation was built (Part A of the M2 checklist) and it was unusable
  from the browser due to a CSRF write bug (**fixed 2026-06-25**). The legacy app's routines,
  group goals, wishlist, badges, allowance, shop depth and reports were **not** ported. M2 was
  rescoped to a **full port** of `~/Documents/new/project-meridian` (proposed D19), with badges
  as a **cross-node** system (proposed D20). See the rewritten `15_Node_Meridian.md` +
  `MILESTONE_2_Checklist.md` (Parts B/C). **Spec rewritten; awaiting owner sign-off before the
  full build.**
