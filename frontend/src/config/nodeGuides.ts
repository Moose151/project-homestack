...
const details: Record<string, Pick<NodeGuide, 'purpose' | 'gettingStarted' | 'capabilities' | 'connections'>> = {
  hub: {
    ...
    connections: ['Receives due work and events from Calendar, Atlas and enabled nodes.', 'Links back to the exact source record where possible.'],
  },
  corners: {
    ...
    connections: ['Fitness activity can be expanded in place.', 'Atlas items, Meridian work and other activity link to their source.'],
  },
  calendar: {
    ...
    connections: ['Atlas shows the same agenda.', 'Enabled nodes publish dates without duplicating their source record.'],
  },
  atlas: {
    ...
    connections: ['Dated items flow to Calendar and Home.', 'Agenda edits Atlas items and standalone events without leaving Atlas.'],
  },
  education: {
  ...
  home_wiki: {
    ...
    connections: ['Complements operational records in Homestead, Pets and Atlas.', 'Can be disabled without deleting its information.'],
  },
  pets: {
  ...
  homestead: {
    ...
    connections: ['House-related bills remain managed in Solace and are only viewed here.', 'Project items and activity can link into Corners.'],
  },
  meridian: {
  ...
  travel: {
    ...
    connections: ['Trip and flight dates publish to Calendar.', 'Booking deadlines create identifiable Atlas to-dos.'],
  },
  solace: {
    ...
    connections: ['Home-related costs can be viewed from Homestead without being managed twice.', 'Due information contributes to Home and notifications.'],
  },
}
...
const catalogueGuides: NodeGuide[] = [
  {
    ...
    purpose: 'A planned capability for quantities, locations, expiry and low-stock reminders. The recommended roadmap places this inside Homestead as optional Stock & storage rather than another permanent destination.',
    gettingStarted: ['This capability is not ready for household use yet.', 'Review the Homestead consolidation proposal before enabling or entering real data.'],
    capabilities: ['Pantry and household consumables', 'Storage locations and quantities', 'Low-stock and expiry reminders'],
    connections: ['Would send replenishment items to Atlas shopping lists.', 'Hearth could read food stock later without owning it.'],
  },
  {
    key: 'assets', label: 'Assets', icon: '🧰', colour: '#64748b', summary: 'Valuables, vehicles, warranties and service history',
    purpose: 'A planned register for vehicles, tools, electronics, valuables and protected documents. Home appliances and warranties already belong to Homestead; the roadmap recommends an optional Assets & vehicles capability there.',
    gettingStarted: ['This capability is not ready for household use yet.', 'Use Homestead for current appliance, warranty and maintenance records.'],
    capabilities: ['Vehicle and equipment records', 'Registration and service history', 'Protected documents and valuations'],
    connections: ['Maintenance dates would publish to Calendar.', 'Financial payments remain owned by Solace.'],
  },
  {
    key: 'hearth', label: 'Hearth', icon: '🍲', colour: '#64748b', summary: 'Meals, recipes and household food planning',
    ...
    gettingStarted: ['This node is not implemented yet.', 'Keep grocery lists in Atlas until its source-owned meal workflow is built.'],
    capabilities: ['Recipe library', 'Meal planning', 'Recipe URL preview and ingredient review'],
    connections: ['Would send groceries to Atlas.', 'Could read Homestead Stock & storage without copying inventory.'],
  },
  {
    key: 'projects', label: 'Projects', icon: '🛠️', colour: '#64748b', summary: 'General multi-step projects',
    purpose: 'A parked general-project concept. Home work belongs in Homestead, travel planning belongs in Travel and lightweight shared work belongs in Atlas; enable a separate node only if a real workflow needs boards, dependencies or broad project budgets.',
    gettingStarted: ['This node is intentionally parked.', 'Use the owning node or an Atlas list for current projects.'],
    capabilities: ['Potential boards and dependencies', 'Potential cross-domain project planning'],
    connections: ['Must not duplicate Homestead room projects or Travel trips.', 'Dated actions would publish through Calendar.'],
  },
  {
    ...
    gettingStarted: ['This node is not implemented yet.', 'Do not store sensitive medical detail in ordinary Atlas notes while the protected workflow is unavailable.'],
    capabilities: ['Health profiles and care history', 'Medication and appointment tracking', 'Protected documents'],
    connections: ['Appointments would project safely to Calendar and Atlas Agenda.', 'Fitness can share permitted high-level activity but must not absorb medical records.'],
  },
]
