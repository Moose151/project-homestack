...
function runSummary(session: FitnessSession): { distance: string; unit: string; duration: string; pace: string } | null {
  ...
  // Round to whole seconds *before* splitting into minutes and seconds — rounding the remainder
  // on its own turns 5:59.7 into "5:60" rather than "6:00".
  const perUnit = Math.round(seconds / distance)
  const pace = `${Math.floor(perUnit / 60)}:${String(perUnit % 60).padStart(2, '0')}`
  return {
