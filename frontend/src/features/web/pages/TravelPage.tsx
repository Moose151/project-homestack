...
import type { Person, TravelBooking, TravelIdea, Trip } from '../../../api/types'
...
type SharedForm = {
  title: string; destination: string; notes: string; start_date: string; end_date: string
...
const blankForm = (): SharedForm => ({ title: '', destination: '', notes: '', start_date: '', end_date: '', colour: '#2B7FD0', status: 'planning', flights_required: false, accommodation_required: false, participant_ids: [], hidden_from_user_ids: [], image_urls: '', rough_cost: '' })
...
function PlanForm({ kind, people, existing, onCancel, onSaved, onError }: {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      if (kind === 'trip') {
        const data = { ...shared, start_date: f.start_date || null, end_date: f.end_date || null, status: f.status }
